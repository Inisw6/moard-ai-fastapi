...
# Dummy Dataset 생성기
def generate_dummy_youtube_dataset(num_samples=500, user_dim=256, goal_dim=64, content_dim=256):
    dataset = []
    for _ in range(num_samples):
        dataset.append({
            ...
            "sentiment_id": np.random.randint(0, 3),
...
# 학습 함수
def train_youtube_dqn(config, dataset):
    ...
    torch.save(q_net.state_dict(), "models/youtube_dqn.pth")
    print("✅ YouTube DQN 모델 저장 완료: models/youtube_dqn.pth")
...
def main():
    ...
    dataset = generate_dummy_youtube_dataset(num_samples=500)
    train_youtube_dqn(config, dataset)
