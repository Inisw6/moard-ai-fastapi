...
# Dummy Dataset 생성기
def generate_dummy_news_dataset(num_samples=500, user_dim=256, goal_dim=64, content_dim=256):
    dataset = []
...
# 학습 함수
def train_news_dqn(config, dataset):
    ...
    torch.save(q_net.state_dict(), "models/news_dqn.pth")
    print("✅ News DQN 모델 저장 완료: models/news_dqn.pth")
...
def main():
    ...
    dataset = generate_dummy_news_dataset(num_samples=500)
    train_news_dqn(config, dataset)
